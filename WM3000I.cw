<!DOCTYPE CW><CW>
<customwidgets>
    <customwidget>
        <class>CBigLetter</class>
        <header location="global">/home/peter/C++/WM3000I/bigletter.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>1</hordata>
            <verdata>1</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XBM.GZ" length="79">789c534e494dcbcc4b554829cdcdad8c2fcf4c29c95030e0524611cd48cd4ccf28010a1797249664262b2467241641a592324b8aa363156c15aab914146aadb90067111b1f</data>
        </pixmap>
        <property type="String">m_sDisplay</property>
        <property type="Int">m_nStartSize</property>
        <property type="Int">m_nAlign</property>
        <property type="Int">m_nFrame</property>
    </customwidget>
    <customwidget>
        <class>cBlinkTextEdit</class>
        <header location="global">/home/peter/C++/WM3000I/blinktextedit.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="PNG" length="1125"></data>
        </pixmap>
        <slot access="public">append(const QString&amp;)</slot>
        <slot access="public">show()</slot>
        <slot access="public">clear()</slot>
        <slot access="public">setBlink(bool)</slot>
    </customwidget>
</customwidgets>
</CW>
